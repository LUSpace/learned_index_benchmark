...
long long art_node4_size(art_node4 *n) {
  long long node_size = sizeof(art_node4);
  for(int i = 0; i < 4; i++) {
    if(n->children[i])
      node_size += art_node_size(n->children[i]);
  }
  return node_size;
}

long long art_node16_size(art_node16 *n) {
  long long node_size = sizeof(art_node16);
  for(int i = 0; i < 16; i++) {
    if(n->children[i])
      node_size += art_node_size(n->children[i]);
  }
...
long long art_node48_size(art_node48 *n) {
  long long node_size = sizeof(art_node48);
  for(int i = 0; i < 48; i++) {
    if(n->children[i])
      node_size += art_node_size(n->children[i]);
  }
...
long long art_node256_size(art_node256 *n) {
  long long node_size = sizeof(art_node256);
  for(int i = 0; i < 256; i++) {
    if(n->children[i])
      node_size += art_node_size(n->children[i]);
  }
...
long long art_node_size(art_node *n) {
  int i;
  union {
    art_node4 *p1;
    art_node16 *p2;
  ...
  if (IS_LEAF(n)) {
    return art_leaf_size(LEAF_RAW(n));
  }
  if(n) {
    switch (n->type) {
      case NODE4:
        p.p1 = (art_node4 *) n;
        return art_node4_size(p.p1);
      case NODE16:
        p.p2 = (art_node16 *) n;
        return art_node16_size(p.p2);
      case NODE48:
        p.p3 = (art_node48 *) n;
        return art_node48_size(p.p3);
      case NODE256:
        p.p4 = (art_node256 *) n;
        return art_node256_size(p.p4);
      default:
        abort();
    }
  }
  return sizeof(art_node);
}

long long art_tree_size(art_tree *t) {
  return sizeof(art_tree) + art_node_size(t->root);
}

long long art_leaf_size(art_leaf * n) {
  return sizeof(art_leaf)+n->key_len;
}