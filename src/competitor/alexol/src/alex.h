    ...
    public:
        ...
        public:
            ...
            bool canAdvance(uint32_t currentEpoch) {
                ...
                return !std::any_of(mThreadSpecificInformations.begin(), mThreadSpecificInformations.end(),
                                    [previousEpoch](
        ...
        public:
            ...
            ~EpochGuard() {
                instance->leaveCriticialSection();
            }
        };
...
// node's parent.
    ...
    public:
        ...
        void bulk_load(const V values[], int num_keys) {
            ...
            bulk_load_node(values, num_keys, root_node_, num_keys, static_cast<double>(min_key),
...
// Correct the traversal path and also lock the parent
        inline bool lock_and_correct_traversal_path(data_node_type *leaf,
                                                    std::vector <TraversalNode> &traversal_path,
                                                    bool left, AlexNode <T, P> *child_node,
                                                    model_node_type **locked_node, bool write_lock = false) const {
...
// Lock parent process may fail? then retry
        ...
        forceinline bool lock_parent_node(
                T key, std::vector <TraversalNode> *traversal_path, AlexNode <T, P> *child_node,
                    ...
                    if (std::abs(bucketID_prediction - bucketID_prediction_rounded) <=
                        ...
                        if (bucketID_prediction_rounded <= bucketID_prediction) {
                            ...
                            if (prev_leaf && prev_leaf->max_limit_ > key) {
                                ...
                                if (!lock_and_correct_traversal_path(leaf, *traversal_path, true, child_node,
                                                                     &locked_node, write_lock)) {
                                    return false;
                                }
                                return true;
                            }
                            ...
                            if (next_leaf && next_leaf->min_limit_ <= key) {
                                model_node_type *locked_node;
                                if (!lock_and_correct_traversal_path(leaf, *traversal_path, false, child_node,
                                                                     &locked_node, write_lock)) {
                                    return false;
                                }
                                return true;
        ...
        bool insert(const T &key, const P &payload) {
            ...
            if (fail) {
                ...
                if (fail == 5) { // Data node resizing
                    // std::cout << "DN expansion with node scaling, key = " << key << std::endl;
                    // 1. Allocate new node
                    data_node_type * node;
                    ...
                    node->resize_from_existing(leaf, data_node_type::kMinDensity_, false, keep_left, keep_right);
                ...
                if (fanout_tree_depth == 0) {
                    // std::cout << "DN expansion with node retraining, key = " << key << std::endl;
                    // 1. Allocate new node
                    data_node_type * node;
                    data_node_type::New_from_existing(reinterpret_cast<void **>(&node), leaf);
                    ...
                    bool keep_left = leaf->is_append_mostly_right();
                    bool keep_right = leaf->is_append_mostly_left();
                    node->resize_from_existing(leaf, data_node_type::kMinDensity_, true, keep_left, keep_right);
                    ...
                    // std::cout << "DN split with node retraining, key = " << key << std::endl;
                    ...
                    if (experimental_params_.allow_splitting_upwards) {
                        ...
                        split_sideways_downwards_without_parent(leaf, fanout_tree_depth,
                                                                used_fanout_tree_nodes, reuse_model, key);
                    }
                }
            }
            //stats_.num_inserts++;
        ...
        void create_two_new_data_nodes_without_parent(data_node_type *old_node, bool reuse_model,
                                                      data_node_type *&split_left, data_node_type *&split_right,
                                                      bool verbose = false) {
        ...
        void create_new_data_nodes_without_parent(
            ...
            int j = 0;
            for (fanout_tree::FTNode &tree_node : used_fanout_tree_nodes) {
                bool keep_left = append_mostly_right && tree_node.left_limit <= old_node->max_key_ &&
                                 old_node->max_key_ < tree_node.right_limit;
                bool keep_right = append_mostly_left && tree_node.left_limit <= old_node->min_key_ &&
        ...
        void split_sideways_downwards_without_parent(
            ...
            if (used_fanout_tree_nodes.empty()) {
                ...
                create_two_new_data_nodes_without_parent(leaf, reuse_model, left_leaf, right_leaf);
            } else {
    ...
    V operator*() const {
      return std::make_pair(cur_leaf_->key_slots_[cur_idx_],
        ...
        public:
            ...
            std::stack<AlexNode < T, P>*>
            node_stack_;  // helps with traversal
