...
#pragma omp parallel for num_threads(thread_num)
  ...
  */
    inline void parse_args(int argc, char **argv) {
        ...
        all_index_type = get_comma_separated(flags, "index_type"); // required
    ...
    void run(index_t *index) {
        ...
        // workload shift first execute half key with write_begin_ratio
        if (workload_shift_test) {
            auto insert_counter = init_table_size;
            auto insert_end = init_table_size + (table_size - init_table_size) / 2;

            std::uniform_real_distribution<> ratio_dis(0, 1);
            std::random_device rd;
            std::mt19937 gen(rd());
#pragma omp parallel for num_threads(thread_num)
            for (int insert_counter = init_table_size; insert_counter < insert_end; insert_counter++) {
                auto paramI = Param(thread_num, 0);
                int i = insert_counter;
                while (true) {
                    auto random_num = ratio_dis(gen);
                    if (random_num > write_ratio_begin) {
                        PAYLOAD_TYPE val;
                        auto key = init_keys[static_cast<size_t>(insert_counter / random_num) % init_table_size];
                        index->get(key, val, &paramI);
                    } else {
                        index->put(keys[insert_counter], insert_counter, &paramI);
                        break;
                    }
                }
            }
...
#pragma omp parallel num_threads(thread_num)
        {
            ...
            //     auto idx = reinterpret_cast<xindexInterface <KEY_TYPE, PAYLOAD_TYPE> *>(index);
            //     cpu_set_t my_set;
            //     CPU_ZERO(&my_set);
...
//        });
        ...
        if (dataset_statistic) {
            ...
            stat.fitness_of_dataset = pgmMetric::PGM_metric(keys, last - keys, error_bound, &stat.mse);
        }
